...
// Shown on account/billing routes when Supabase/Stripe env isn't configured.
...
export function SetupPending({ what = "Accounts" }: { what?: string }) {
  return (
    <Section className="max-w-xl py-24 text-center">
      <div className="mx-auto grid h-12 w-12 place-items-center rounded-xl bg-amber-100 text-amber-700">⚙️</div>
      <h1 className="mt-4 text-2xl font-bold text-slate-900">{what} are being set up</h1>
      <p className="mt-2 text-slate-600">
        The team dashboard and billing turn on once this deployment is connected to its database and payment keys.
        Everything you need today is already free and live — no account required.
      </p>
